...
if __name__ == '__main__':
    ...
    spatial_transform = transforms.TranslationTransform()
...
#   rendered_images = blend(render, np.array([0.7, 0.2, 0.2]))
    rendered_images = blend(render, np.array([0, 0, 0]))
    ...
    for i in range(number_of_iterations):
        ...
        if i == 0 or (i+1) % 100 == 0:
            ...
            print('gradients', np.linalg.norm(outputs[9]))

    # show images
    input_image = input_image[0]
    output_image = outputs[1][0]
    imutils.imshow((input_image, output_image, output_image - input_image, input_image - output_image))
